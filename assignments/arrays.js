...
// for(let i=0; i <= inventory.length;  i++){}
// if(inventory[i].car_model == carModel){

// } else if (inventory[i].car_year == carYear){

// }  else (inventory[i].car_make == carMake){
...
console.log("Last car is " + inventory[lastCar].car_year +inventory[lastCar].car_make + inventory[lastCar].car_model   );
...
for (let i = 0; i < inventory.length; i++) {
      if (carModels.includes(inventory[i].car_model)  === false) { 
...
for (let i = 0; i < carYears.length; i++) {
  if (carYears[i]  < 2000) { 
    oldCars.push(carYears[i].car_model); 
  }
}
